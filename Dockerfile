...
LABEL maintainer="Vikram Somavaram <vikramsomavaram@gmail.com>"
...
COPY arangobackup.sh /var/arangobackup/arangobackup.sh

# Give full permissions to the script
RUN chmod 777 /var/arangobackup/arangobackup.sh \
    && chmod 777 /entrypoint.sh \
    && mkdir -p /mnt/data/dump/

ENTRYPOINT ["/entrypoint.sh"]

# Execute the backup script during startup
CMD /var/arangobackup/arangobackup.sh
